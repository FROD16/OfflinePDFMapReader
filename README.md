...
### Device direction
The field map now displays a compass direction indicator using the Device Orientation API. On browsers that require sensor permission (notably iOS Safari), tap **Locate me** and allow orientation access. The direction indicator uses `webkitCompassHeading` where available and standard absolute orientation data elsewhere. Device orientation is separate from GPS location; `GeolocationCoordinates.heading` is travel direction and may be null while stationary.


## Location permission behavior (v0.5)

The app requests device location each time the page is loaded. If the browser has already granted location permission, no prompt is shown. The compass/orientation permission is requested only when **Locate me** is pressed, because some mobile browsers require a user gesture for sensor permission. A compass failure does not block GPS.
