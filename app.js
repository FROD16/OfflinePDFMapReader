...
function fitStage() {
  ...
  const fit = Math.min((box.width - 12) / pageWidth, (box.height - 12) / pageHeight);
  state.fitZoom = Math.max(0.25, Math.min(1.8, fit));
...
async function renderPdf(bytes, fileName) {
  ...
  await savePdfToIndexedDb({ fileName, bytes, geo });
...
els.locateButton.addEventListener("click", startGps);
