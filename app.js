...
function startGps() {
  if (!("geolocation" in navigator)) {
    els.gpsStatus.textContent = "Unavailable";
    setMessage("This browser does not expose device geolocation.", "warn");
    return;
  }
  if (!window.isSecureContext) {
    els.gpsStatus.textContent = "Unavailable";
    setMessage("Location requires HTTPS (or localhost). Open the GitHub Pages URL directly.", "warn");
    return;
  }
  if (state.gpsWatchId !== null) navigator.geolocation.clearWatch(state.gpsWatchId);
  els.gpsStatus.textContent = "Requesting location…";
...
els.locateButton.addEventListener("click", () => {
  // GPS and compass permissions are independent. Keep GPS working even if
  // orientation permission is unavailable or requires a separate gesture.
  startGps();
  startOrientation();
});
...
setNetworkStatus();
// Request location on each page load. Browsers decide whether to show the
// permission prompt; if permission was already granted, the request proceeds
// without prompting. Compass permission is requested only from the Locate me
// button because some mobile browsers require a user gesture.
startGps();
loadStoredPdf();
